<script lang="ts">
    ...
    function getChartData() {
        ...
        const values: number[] = labels.map(label => {
            ...
            const value = measurement.calculated_value > 1000
...
<section id="container">
        ...
        <div class="data">
            <div class="txt-group">
                ...
                <br>
                <p class="xl">Wskazanie:
                    {#if data.device.calculatedValue === 0}
                        <span class='possibly-out-of-date tooltip'>
                        <span class="tooltiptext xs">Podana wartość może być przestarzała, zobacz zdjęcia aby odczytać najnowszą wartość</span>
                            {data.device.calculatedValue !== 0 ? data.device.calculatedValue : deviceMeasurements[deviceMeasurements.length - 1].calculated_value}
                        </span>
                    {:else}
                        {data.device.calculatedValue !== 0 ? data.device.calculatedValue : deviceMeasurements[deviceMeasurements.length - 1].calculated_value}
                    {/if}
