export async function load({ params, cookies }) {
    ...
    const imageResponse = await fetch(`https://${data.serwer}/api/integrations/ocr/${params.id}/images`, {
        method: 'GET',
        headers: {
            authorization: `Bearer ${api_token}`,
            'X-Accept-Version' : '3'
    ...
    let device = {}
    const deviceResponse = await fetch(`https://${data.serwer}/api/v3/channels/${params.id}?include=iodevice,connected,state,location`, {
        method: 'GET',
        headers: {
            authorization: `Bearer ${api_token}`
        }
